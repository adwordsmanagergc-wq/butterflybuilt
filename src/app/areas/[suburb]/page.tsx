...
export function generateMetadata({ params }: { params: { suburb: string } }) {
  ...
  if (!sub) return {};
  // No per-suburb hero image — use the site's default OG so the card is
  // generic across every suburb page.
...
export default function SuburbPage({ params }: { params: { suburb: string } }) {
  ...
  return (
    <>
      ...
      {/* Generic workshop gallery — same flowing photo strip used site-wide.
          Photos are our recent Sydney work, not tied to this particular
          suburb, so we don't imply any project shown was in {sub.name}. */}
      <section className="py-16 sm:py-24">
        <div className="container-prose">
          ...
          <p className="mt-4 max-w-prose2 text-cream/70">
            A rolling look at what&apos;s currently coming out of the Butterfly
            Built workshop — projects from across Sydney, not tied to any one
            street or suburb.
          </p>
        </div>
